            ...
            \begin{enumerate}
                ...
                \end{center}
                        ...
                        \item For each of the following, approximate the area under the function $f(x)$ using $n$ rectangles of equal width and right-hand endpoints. These problems relate closely to several problems you did in previous work.
                            \begin{enumerate}
                                \item $ f(x) = \frac{1}{5}x+4$ on the interval $[0,10]$ and $n=10$  
                                \item $  f(x) = x^2-4x+5$ on the interval $[1, 5]$ and $n=8$ \\ %\textcolor{red}{(the summation problem is $ \sum_{j=1}^{8} \frac{1}{2}\left[\left(1+\frac{j}{2} \right)^2 -4\left( 1+ \frac{j}{2}\right) + 5\right]$)}
                            \end{enumerate}
               ...
               % \textcolor{red}{Faan Tone, I put the following summation problem in Week 1 under the same instructions as the two summations mentioned just above. $ \sum_{j=1}^{n} \frac{4}{n}\left[ \left(\frac{j}{n}\right)^2 - \frac{j}{n} + 1 \right]$}
                \item Find the value of $ \int_{0}^{3} \sqrt{9-x^2}\,dx$ by graphing the appropriate region and calculating the area using basic geometry facts.
                \item If $ \int_a^b f(x)\,dx = c$, what does $ \int_a^b 77f(x)\,dx$ equal? Explain.
                \item If $ \int_a^b f(x)\,dx = c$, and $ \int_a^b g(x)\,dx = d$, what does $ \int_a^b f(x)+g(x)\,dx$ equal? Explain.
                \item If $a < b < c$,  $ \int_a^c f(x)\,dx = 15$, and $ \int_c^b f(x)\,dx = 23$, then what does $ \int_a^b f(x)\,dx$ equal?
                    \item Suppose I tell you that the definite integral
$ \int_0^x f(t)\,dt = e^{x^3}$.
\begin{enumerate}
    \item Find $ \int_0^2 f(t) \,dt$.
    \item Find $ \int_1^3 f(t)\,dt$.
    \item Find $ \int_{-1}^1 f(t)\,dt$.
\end{enumerate}
                \end{enumerate}
